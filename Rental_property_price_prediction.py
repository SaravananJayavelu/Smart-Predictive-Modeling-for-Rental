...
st.title(':red[Smarty] - :blue[Home Rental Price Predictor 🚀]')

@st.cache_resource
def load_model():
    model = pickle.load(open(r'C:\GUVI\Placemnet Sepecial Class\Project\2_Smart Predictive Modeling for Rental\regression_model.pkl', 'rb'))
    return model

model = load_model()

transformer = pickle.load(open(r'C:\GUVI\Placemnet Sepecial Class\Project\2_Smart Predictive Modeling for Rental\transformer.pkl','rb'))


df = pd.read_csv(r'C:\GUVI\Placemnet Sepecial Class\Project\2_Smart Predictive Modeling for Rental\analysis_df.csv')
...
if submit:
    ...
    st.subheader(":green[Predicted Home Rental Price]: #{result[0]}")
